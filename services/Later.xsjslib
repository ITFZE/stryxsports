var later = function () {
  ...
  later.array.next = function (val, values, extent) {
    var cur, zeroIsLargest = extent[0] !== 0,
      nextIdx = 0;
  ...
  later.compile = function (schedDef) {
    ...
    return {
      start: function (dir, startDate) {
        ...
        var result, nextVal = later.array[dir + "Invalid"],
  ...
  later.schedule = function (sched) {
    ...
    function getInstances(dir, count, startDate, endDate, isRange) {
      var compare = compareFn(dir),
        ...
        next, end, results = [],
        isForward = dir === "next",
        lastResult, rStart = isForward ? 0 : 1,
        rEnd = isForward ? 1 : 0;
  ...
  later.parse.recur = function () {
    var schedules = [],
      exceptions = [],
      cur, curArr = schedules,
      curName, values, every, modifier, applyMin, applyMax, i, last;
